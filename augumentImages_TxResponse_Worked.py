...
#image needs to be centered, and are then re-centered again after division
    ...
    timg1 = np.reshape(timg1,(96,96))
    timg2 = np.reshape(timg2,(96,96))
    timg3 = np.reshape(timg3,(96,96))

    return center_image(np.zeros((96,96)), timg1), center_image(np.zeros((96,96)), timg2), center_image(np.zeros((96,96)), timg3)


def saveFiles(file_path, name, onames, afiles, amap, bmap, dmap):
    ''' Saves the files
        Args:
            @file_path: location to save files
            @name: what prefix of the file name should be 
            @onames: what the parameter names are 
            @afiles: a list of of maps and the patient names, this is mainly to obtain patient names for naming the file
            @amap/bmap/dmap: parameter maps
    '''
    for  i, (afile,amap,bmap,dmap) in enumerate(zip(afiles,amap,bmap,dmap)):
        
        patientname = afile[2]
        label = afile[1]
        af = file_path + '/%s_%s_%s'%(name,onames[0],patientname)
        np.save(af,[amap, label, patientname])
        af = file_path + '/%s_%s_%s'%(name,onames[1],patientname)
        np.save(af,[bmap, label, patientname])
        af = file_path + '/%s_%s_%s'%(name,onames[2],patientname)
...
print("Finished Augmentation woo")